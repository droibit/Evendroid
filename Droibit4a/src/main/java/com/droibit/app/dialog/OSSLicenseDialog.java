...
public class OSSLicenseDialog {
	...
	private final int mResChangelog;

	protected DialogInterface.OnDismissListener mOnDismissListener;

	public OSSLicenseDialog(Context context, int resChangelog) {
		mContext = context;
		mResChangelog = resChangelog;
	}
	...
	// Parse a the release tabName and appends it to the changelog builder
	...
	public OSSLicenseDialog setOnDismissListener(
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if ((eventType == XmlPullParser.START_TAG)
					...
					if ((version == 0) || (versioncode == version)) {
						...
						releaseFound = true; // At lease one release tabName has
												// been parsed.
					}
				}
				eventType = xml.next();
	...
	public String getHTML() {
		...
		return getHTMLChangelog(mResChangelog, resources, 0);
	}
	...
	public Dialog makeDialog(final int version) {
		...
		// Create html change log
		final String htmlChangelog = getHTMLChangelog(mResChangelog, resources,
				version);
