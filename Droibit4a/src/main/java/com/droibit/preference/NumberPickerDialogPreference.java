...
public class NumberPickerDialogPreference extends DialogPreference {
	...
	public NumberPickerDialogPreference(Context context, AttributeSet attrs) {
		...
		final TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
				R.styleable.NumberPickerDialogPreferenceAttr, 0, 0);
		try {
			setMinValue(a.getInteger(
					R.styleable.NumberPickerDialogPreferenceAttr_min,
					DEFAULT_MIN_VALUE));
			setMaxValue(a.getInteger(
					R.styleable.NumberPickerDialogPreferenceAttr_android_max,
					DEFAULT_MAX_VALUE));
		} finally {
			a.recycle();
		}
	...
	protected void onBindDialogView(View view) {
		...
		final TextView dialogMessageText = (TextView) view
				.findViewById(R.id.text_dialog_message);
		if (!TextUtils.isEmpty(getDialogMessage())) {
			dialogMessageText.setText(getDialogMessage());
		} else {
			dialogMessageText.setVisibility(View.GONE);
		}
